...
struct Image * loadImage(const char* filename)
{
  ...
  int retval;
  retval = fread(&hdr1,sizeof(struct ImageHeader),1,file);
  if (retval != 1)
  {
    fclose(file);
    return NULL;
  ...
  retval = fread(img -> comment,sizeof(char),hdr1.comment_len,file);
  if (retval != hdr1.comment_len)
  {
    fclose(file);
    return NULL;
  }
  if (img -> comment[hdr1.comment_len-1] != 0)
  {
    fclose(file);
    return NULL;
  }
  retval = fread(img -> data,sizeof(uint8_t),hdr1.width * hdr1.height,file);
  if (retval != (hdr1.width * hdr1.height))
  {
    fclose(file);
    return NULL;
  }
