...
int isValidState(const char * state)
{
  ...
  char temp[len + 1];
  const char * correct = "-123456789ABCDEF";
  strcpy(temp,state);
  qsort(temp,len,sizeof(char),comp_char);
  if (strcmp(correct,temp))
  {
    return FALSE;
  }
  return TRUE;
...
{    
  ...
  switch(m)
  {
    ...
    case 'D': y = 1;
      break;
    default:
      printf("Error: %c is not a valid move\n",m);
      return FALSE;
  }
  char * str_pos = strstr(state,"-");
  if (str_pos == NULL)
  {
    printf("Error: %s is not a valid state",state);
    return FALSE;
  }
  int pos = str_pos - state;
  int row = pos / SIDELENGTH;
  int col = pos % SIDELENGTH;
  if(row + y < 0 || row + y >= SIDELENGTH)
  {
    return FALSE;
  }
  if(col + x < 0 || col + x >= SIDELENGTH)
  {
    return FALSE;
  }
  int new_pos = (row+y)*SIDELENGTH + (col+x);
			   ...
			   const char * moves)
{
  ...
  int comp = strcmp(state,node -> state);
  if (comp < 0)
...
MoveTree * MoveTree_find(MoveTree * node, const char * state)
{
  if(node == NULL)
    return NULL;
  int comp = strcmp(node -> state, state);
  if(comp < 0)
...
{ 
  void generate_help(MoveTree*,const char*,int,char*,int);
  MoveTree * tree = MoveTree_create(state, "");
  char * movelist = malloc((n_moves+1) * sizeof(char));
  movelist[n_moves] = '\0';
  generate_help(tree, state,n_moves, movelist, 0);
  free(movelist);
  return tree;
}

void generate_help(MoveTree * root,const char * state,int n_moves,char * movelist, int ind)
{    
  const char possible[] = { 'U', 'D', 'L', 'R' };
  int num_possible = sizeof(possible) / sizeof(char);
  if(ind == n_moves)
    return;
  int i;
  ...
  {
    char m = possible[i];
    char * state_cpy = strdup(state);
      if(move(state_cpy, m))
      {
        movelist[ind] = m;
        movelist[ind+1] = '\0'; // Terminates the movelist
        MoveTree_insert(root, state_cpy, movelist);
        generate_help(root, state_cpy,n_moves, movelist,ind + 1);
      } 
      else
      {
      }
